...
public class PostDao {
    ...
    // 탐지한 스키마 정보를 캐시로 저장하는 필드(volatile로 멀티스레드에서 가시성 보장)
    private volatile SchemaInfo cachedPost;

    private DataSource requireDs() {
        // JdbcTemplate에서 DataSource를 꺼내오는 헬퍼 메서드
    ...
    private SchemaInfo ensurePostResolved() {
        ...
        synchronized (this) {
            ...
            try (Connection conn = requireDs().getConnection()) {
                ...
                si.board = pick(cols, "board_code", "board_uuid", "boardcd", "board");
                // 보드 식별 컬럼 후보들 중 선택
    ...
    private String findBoardUuidByCode(String boardCode) {
        ...
        final String sql =
                "SELECT uuid FROM board WHERE board_code = ? AND is_active = 1";
        // 활성화된(board.is_active=1) 게시판 중에서 uuid 조회
    ...
    private boolean boardColumnIsUuid(SchemaInfo s) {
        ...
        return s.board != null && "board_uuid".equalsIgnoreCase(s.board);
        // true면 board_uuid 스키마
    }
    ...
    private void appendSearchConditions(
        ...
        if (!hasText(kw)) return;
        // 키워드가 비어 있으면 아무 조건도 추가하지 않고 종료

        String likeValue = "%" + kw + "%";
        // LIKE 검색에 사용할 패턴 값(양쪽에 % 붙여줌)

        List<String> cols = new ArrayList<>();
        // 어떤 컬럼을 LIKE로 검색할지 담을 리스트
    ...
    public List<PostDto> findByBoard(String code) {
        ...
        if (boardColumnIsUuid(s)) {
            // 게시글 테이블의 board 컬럼이 board_uuid 타입인 경우
            String sql =
                    "SELECT p.* " +
                    "FROM " + s.table + " p " +
    ...
    private PostDto mapRow(ResultSet rs, SchemaInfo s) throws SQLException {
        ...
        if (s.board != null && !boardColumnIsUuid(s)) {
            try {
                d.setBoardCode(rs.getString(s.board));
            } catch (SQLException ignore) {}
        }

        if (s.title != null) {
            try {
                d.setTitle(rs.getString(s.title));
            } catch (SQLException ignore) {}
        }

        if (s.content != null) {
            try {
                d.setContent(rs.getString(s.content));
            } catch (SQLException ignore) {}
        }

        if (s.fileUrl != null) {
            try {
                d.setFileUrl(rs.getString(s.fileUrl));
            } catch (SQLException ignore) {}
        }

        if (s.fileType != null) {
            try {
                d.setFileType(rs.getString(s.fileType));
            } catch (SQLException ignore) {}
        }

        if (s.fileName != null) {
            try {
                d.setFileName(rs.getString(s.fileName));
            } catch (SQLException ignore) {}
        }

        if (s.fileContentType != null) {
            try {
                d.setFileContentType(rs.getString(s.fileContentType));
            } catch (SQLException ignore) {}
        }

        if (s.fileListJson != null) {
            try {
                d.setFileListJson(rs.getString(s.fileListJson));
            } catch (SQLException ignore) {}
        }

        if (s.writerId != null) {
            try {
                d.setWriterId(rs.getString(s.writerId));
            } catch (SQLException ignore) {}
        }

        if (s.writerName != null) {
            try {
                d.setWriterName(rs.getString(s.writerName));
            } catch (SQLException ignore) {}
        }
    ...
    public int update(PostDto d) {
        ...
        if (s.updatedAt != null) sb.append(", ").append(s.updatedAt).append(" = NOW()");
