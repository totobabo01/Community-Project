...
window.TRAM_ROUTE_FULL_HD = [
    ...
    // -------------------------------------------------------
    // [기타 구간들]
    // (중간 waypoint들은 경로용이므로 검색에는 영향 없음)
    // -------------------------------------------------------
];
...
//    → Bus 정류장과 완전히 동일한 구조
// =========================================================
window.TRAM_STOPS = window.TRAM_ROUTE_FULL_HD.filter((p) => p.type === 'station' && p.id && p.lat && p.lng).map((p) => ({
    stopId: String(p.id),
    id: String(p.id),
    name: String(p.name),
    lat: Number(p.lat),
    lng: Number(p.lng), // ✅ 핵심 수정
    section: p.section,
    mode: 'TRAM',
}));
...
// 디버그 확인용 (콘솔)
// =========================================================
console.log('[TRAM] stops loaded:', TRAM_STOPS.length);
console.log('[TRAM] example:', TRAM_STOPS[0]);
